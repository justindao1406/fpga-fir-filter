...
![PYNQ-Z2 FIR test with pass and done LEDs illuminated](docs/images/fpga_result.png)
...
-> Open `vivado/fir_filter_v1/fir_filter_v1.xpr`.  
-> Set `fir_filter_q15_tb` as the simulation top to run the impulse-response test.  
-> Run Behavioral Simulation and confirm that the console reports zero mismatches.  
-> Set `fir_filter_q15_wave_tb` as the simulation top to run the 480-sample two-tone test.  
-> Run Behavioral Simulation again and confirm that the test passes with zero mismatches.

### FPGA Deployment

-> Set `fir_filter_q15_fpga_top` as the design top module.  
-> Ensure that `pynq_z2_fir_demo.xdc` is enabled.  
-> Run synthesis and implementation.  
-> Generate the bitstream.  
-> Connect and power on the PYNQ-Z2 to the computer.  
-> Click "Open Hardware Manager", then "Open Target" and select XC7Z020 as the device and program it using the generated bitstream.  
-> Confirm `led_done` and `led_pass` are turned on with `led_fail` being turned off.  
-> Press the reset button to clear the LEDs and restart the onboard test.
